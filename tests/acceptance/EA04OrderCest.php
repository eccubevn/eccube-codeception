...
class EA04OrderCest
{
    ...
    public function order_受注削除(\AcceptanceTester $I)
    {
        ...
        $OrderListPage->一覧_acceptDeleteModal(1);

        $I->see('受注情報を削除しました', ['css' => '#page_admin_order_page > div.c-container > div.c-contentsArea > div.alert']);
        $I->assertNotEquals($OrderNumForDel, $OrderListPage->一覧_注文番号(1));

        // 削除キャンセル
        $OrderNumForDontDel = $OrderListPage->一覧_注文番号(1);
        $OrderListPage->一覧_削除(1);
        $OrderListPage->一覧_cancelDeleteModal(1);
